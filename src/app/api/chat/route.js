// Hey there! This is the server-side API handler for our chatbot in Next.js 15 App Router.
...
export async function POST(req) {
  try {
    ...
    const { message, language, state } = await req.json();
    console.log('Received request:', { message, language, state });

    // Validate request body
    if (!language) {
      console.error('Missing language in request body');
      return NextResponse.json({ error: 'Language is required' }, { status: 400 });
    }

    // Define products and services for easy reference
    ...
    switch (newState.step) {
      ...
      case 'book_care':
        ...
        response = getResponse(
          'Please provide your name and email for the appointment (e.g., John Doe, john@example.com).',
          'कृपया अपॉइंटमेंट के लिए अपना नाम और ईमेल प्रदान करें (उदा., John Doe, john@example.com).',
          'कृपया भेटीसाठी तुमचे नाव आणि ईमेल द्या (उदा., John Doe, john@example.com).'
        );
    ...
    if (!state || state.step === 'main_menu') {
      ...
      if (lowerMessage.includes('product') || message === '1') {
        console.log('Transitioning to products step for message:', message);
        ...
        newState.options = products.map((_, i) => (i + 1).toString());
      } else if (lowerMessage.includes('service') || message === '2') {
        console.log('Transitioning to services step for message:', message);
        ...
        newState.options = services.map((_, i) => (i + 1).toString());
      } else if (lowerMessage.includes('support') || lowerMessage.includes('amc') || message === '3') {
        console.log('Transitioning to support_amc step for message:', message);
        ...
        newState.options = ['1', '2', '3', '4', '5'];
      } else if (lowerMessage.includes('contact') || lowerMessage.includes('help') || message === '4') {
        console.log('Transitioning to contact_care step for message:', message);
    ...
    console.log('Sending response:', { response, state: newState });
    return NextResponse.json({ response, state: newState });
  } catch (error) {
